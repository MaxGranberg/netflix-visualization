...
// Determine the directory name and dataset path
...
async function indexData () {
  try {
    ...
    for (let i = 0; i < dataset.length; i += chunkSize) {
      // Slice the dataset into chunks of chunkSize
      const chunk = dataset.slice(i, i + chunkSize)

      // Prepare the bulk API request body
      const body = chunk.flatMap(doc => {
        // Process and format the date_added field
        if (doc.date_added) {
          try {
      ...
      // Use the bulk API to index documents in chunks
 ...
 * Refresh the index to ensure all operations are completed.
 */
